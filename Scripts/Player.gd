...
func _draw() -> void:
	if path.size() > 0:
		for i in range(path.size() - 1):
			var start = Map.map_to_local(path[i])-position
			var end = Map.map_to_local(path[i + 1])-position
			draw_line(start, end, Color(1, 0, 0), 2) # Draw the path in red with a width of 2
